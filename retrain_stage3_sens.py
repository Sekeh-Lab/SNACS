...
def gen_mask(prune_percent, upper_prune_limit, parent_key, children_key, clusters, children_clusters, labels_file, labels_children_file, parents_file, device, final_weights, save_children):
    ...
    for num_layers in range(len(parent_key)):
        ...
        # END IF
        #if parent_k == 'conv3.weight'or parent_k == 'conv4.weight' or parent_k == 'conv7.weight'or parent_k == 'conv8.weight'or parent_k == 'conv11.weight':
        #    plt.hist(reg)
        #    plt.show()
...
def train(Epoch, Batch_size, Lr, Dataset, Dims, Milestones, Rerun, Opt, Weight_decay, Model, Gamma, Nesterov, Device_ids, Retrain, Retrain_mask, Labels_file, Labels_children_file, prune_percent, parent_key, children_key, parent_clusters, children_clusters, upper_prune_limit, baseline_prune_per, samples_per_class, save_children):
    ...
    mask, true_prune_percent, total_count = gen_mask(prune_percent, upper_prune_limit, parent_key, children_key, parent_clusters, children_clusters, Labels_file, Labels_children_file, Retrain_mask, device, load_checkpoint(Retrain), save_children)
